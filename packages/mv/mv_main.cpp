...
QJsonObject get_spec()
{
    ...
    /*
    {
        ProcessorSpec X("mv.synthesize_timeseries", "0.12");
    ...
    */
    {
        ProcessorSpec X("mv.combine_firing_segments", "0.13");
    ...
    {
        ProcessorSpec X("mv.extract_firings", "0.2");
        X.addInputs("firings");
    ...
    {
        ProcessorSpec X("mv.concat_timeseries", "0.11");
    ...
    {
        ProcessorSpec X("mv.compute_whitening_matrix", "0.11");
        X.addInputs("timeseries_list");
    ...
    {
        ProcessorSpec X("mv.whiten_clips", "0.1");
    ...
    {
        ProcessorSpec X("mv.apply_whitening_matrix", "0.1");
    ...
    */
    ...
    {
        ProcessorSpec X("mv.mv_extract_clips", "0.1");
        X.addInput("timeseries");
    ...
    {
        ProcessorSpec X("mv.mv_extract_clips_features", "0.1");
        X.addInputs("timeseries", "firings");
    ...
    {
        ProcessorSpec X("mv.compute_templates", "0.11");
        X.addInputs("timeseries", "firings");
        X.addOutputs("templates_out");
        X.addRequiredParameters("clip_size");
    ...
    {
        ProcessorSpec X("mv.create_firings", "0.1");
    ...
    {
        ProcessorSpec X("mv.combine_firings", "0.1");
    ...
    {
        ProcessorSpec X("mv.apply_timestamp_offset", "0.1");
        X.addInputs("firings");
        X.addOutputs("firings_out");
    ...
    {
        ProcessorSpec X("mv.link_segments", "0.1");
    ...
    {
        ProcessorSpec X("mv.cluster_metrics", "0.11");
        X.addInputs("timeseries", "firings");
    ...
    {
        ProcessorSpec X("mv.isolation_metrics", "0.15j");
        X.addInputs("timeseries", "firings");
        X.addOutputs("metrics_out");
    ...
    /*
    {
        ProcessorSpec X("mv.extract_firings", "0.11");
        X.addInputs("firings");
        X.addOutputs("firings_out");
    ...
    */
    {
        ProcessorSpec X("mv.combine_cluster_metrics", "0.1");
        X.addInputs("metrics_list");
        X.addOutputs("metrics_out");
        processors.push_back(X.get_spec());
    }
    {
        ProcessorSpec X("mv.split_firings", "0.15");
    ...
    {
        ProcessorSpec X("mv.concat_event_times", "0.1");
    ...
    {
        ProcessorSpec X("mv.confusion_matrix", "0.17");
    ...
    {
        ProcessorSpec X("mv.mask_out_artifacts", "0.1");
        X.addInputs("timeseries");
        X.addOutputs("timeseries_out");
    ...
    {
        ProcessorSpec X("mv.mv_subfirings", "0.1");
        X.addInputs("firings");
        X.addOutputs("firings_out");
    ...
    {
        ProcessorSpec X("mv.mv_compute_amplitudes", "0.1");
        X.addInputs("timeseries", "firings");
        X.addOutputs("firings_out");
        processors.push_back(X.get_spec());
    }
...
int main(int argc, char* argv[])
{
    ...
    /*
    else if (pname == "mv.synthesize_timeseries") {
        QString firings = CLP.named_parameters["firings"].toString();
    ...
    */
    else if (pname == "mv.combine_firing_segments") {
        QString timeseries = CLP.named_parameters["timeseries"].toString();
    ...
    else if (pname == "mv.extract_firings") {
        QString firings = CLP.named_parameters["firings"].toString();
        QString metrics = CLP.named_parameters["metrics"].toString();
        QString firings_out = CLP.named_parameters["firings_out"].toString();
    ...
    else if (pname == "mv.concat_timeseries") {
    ...
    else if (pname == "mv.compute_whitening_matrix") {
        QStringList timeseries_list = MLUtil::toStringList(CLP.named_parameters["timeseries_list"]);
    ...
    else if (pname == "mv.apply_whitening_matrix") {
        QString timeseries = CLP.named_parameters["timeseries"].toString();
        QString whitening_matrix = CLP.named_parameters["whitening_matrix"].toString();
        QString timeseries_out = CLP.named_parameters["timeseries_out"].toString();
        Whiten_opts opts;
    ...
    */
    ...
    else if (pname == "mv.mv_extract_clips") {
        QStringList timeseries_list = MLUtil::toStringList(CLP.named_parameters["timeseries"]);
        QString firings = CLP.named_parameters["firings"].toString();
        QString clips_out = CLP.named_parameters["clips_out"].toString();
        QStringList channels_str = CLP.named_parameters["channels"].toString().split(",", QString::SkipEmptyParts);
    ...
    else if (pname == "mv.mv_extract_clips_features") {
        QString timeseries = CLP.named_parameters["timeseries"].toString();
        QString firings = CLP.named_parameters["firings"].toString();
    ...
    else if (pname == "mv.compute_templates") {
        QStringList timeseries_list = MLUtil::toStringList(CLP.named_parameters["timeseries"]);
        QString firings = CLP.named_parameters["firings"].toString();
        QString templates_out = CLP.named_parameters["templates_out"].toString();
        int clip_size = CLP.named_parameters["clip_size"].toInt();
    ...
    else if (pname == "mv.create_firings") {
        QString event_times = CLP.named_parameters["event_times"].toString();
    ...
    else if (pname == "mv.combine_firings") {
        QStringList firings_list = MLUtil::toStringList(CLP.named_parameters["firings_list"]);
        QString firings_out = CLP.named_parameters["firings_out"].toString();
    ...
    else if (pname == "mv.apply_timestamp_offset") {
        QString firings = CLP.named_parameters["firings"].toString();
        QString firings_out = CLP.named_parameters["firings_out"].toString();
    ...
    else if (pname == "mv.link_segments") {
        QString firings = CLP.named_parameters["firings"].toString();
    ...
    else if (pname == "mv.cluster_metrics") {
        QString timeseries = CLP.named_parameters["timeseries"].toString();
        QString firings = CLP.named_parameters["firings"].toString();
    ...
    else if (pname == "mv.isolation_metrics") {
        QStringList timeseries_list = MLUtil::toStringList(CLP.named_parameters["timeseries"]);
        QString firings = CLP.named_parameters["firings"].toString();
        QString metrics_out = CLP.named_parameters["metrics_out"].toString();
    ...
    else if (pname == "mv.combine_cluster_metrics") {
    ...
    else if (pname == "mv.split_firings") {
        QStringList timeseries_list = MLUtil::toStringList(CLP.named_parameters["timeseries_list"]);
        QString firings = CLP.named_parameters["firings"].toString();
    ...
    */
    ...
    else if (pname == "mv.concat_event_times") {
    ...
    else if (pname == "mv.compute_amplitudes") {
    ...
    else if (pname == "mv.confusion_matrix") {
    ...
    else if (pname == "mv.mask_out_artifacts") {
        QString timeseries = CLP.named_parameters["timeseries"].toString();
        QString timeseries_out = CLP.named_parameters["timeseries_out"].toString();
    ...
    else if (pname == "mv.mv_subfirings") {
        QString firings = CLP.named_parameters["firings"].toString();
        QString firings_out = CLP.named_parameters["firings_out"].toString();
    ...
    else if (pname == "mv.mv_compute_amplitudes") {
        QString timeseries = CLP.named_parameters["timeseries"].toString();
        QString firings = CLP.named_parameters["firings"].toString();
        QString firings_out = CLP.named_parameters["firings_out"].toString();
